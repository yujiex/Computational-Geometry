...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\linewidth]{triAndPath2.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\linewidth]{pinchEdge.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\linewidth]{pinchEdgeDouble.png}
  \caption{When an edge is pinched}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.5\linewidth]{pinchFace.png}
...
\begin{definition}(Cell-chain: a sequence of 1-d numbered simplices
  ...
  \item Each $\gamma_i$ is consecutively numbered.
    \begin{figure}[h!]
      \centering
      \includegraphics[width=0.5\linewidth]{cellChain.png}
      \caption{Cell chain}
      \label{fig:cellChain}
    \end{figure}
  \end{enumerate}
...
Topological representations are required to ``represent topological
structure up to equivalence''

Ordering is also an important property to be included in a
representation.

To ``have access to the topological dual'' is also handy.

``The dual of a subdivided d-dimensional object is a subdivision of the
same object produced by replacing every k-dimensional cell by a (d -
k)-dimensional cell, while maintaining the corresponding incidence
relations.''
\lecture{lec}{2D closest pair using hashing and randomization}{Gary
  Miller}{Wed, Oct 14, 2015}
\section{2D Closest Pair}
\begin{enumerate}
\item Problem definition (the Closest Pair Problem)
  \begin{itemize}
  \item Input: $P \subseteq \mathbb{R}^2, P \subseteq UnitBox, |P| = n$
  \item Output:
    $CP(P) = \min\{p, q \in P \land p \neq q \mid ||p - q||\}$
  \end{itemize}
\item Solve it by hashing: placing points into boxes
  \begin{itemize}
  \item Idea: partition the UnitBox into boxes of
    $\alpha \times \alpha$
    \begin{figure}[h!]
      \centering
      \includegraphics[width=0.5\linewidth]{cpHashing.png}
      \caption{Construction of the hashing grid}
      \label{fig:cpHashing}
    \end{figure}
  \item The problem is there will be too many boxes to store in
    memory. Say if $\alpha = 10^{-20}$, then there will be
    ${1 \over \alpha^2} = 10^{40}$ boxes.
    
    To solve this problem, make the names of the boxes be the key
    space (i.e. for each box, the key is $(x, y)$, with
    $x, y \in [1, 10^{20}$ x, y are integers. Make a dynamic hash
    table of size $O(n)$.
  \end{itemize}
\begin{theorem}
  Hash points ``into'' its box is $O(1)$ time.
\end{theorem}
\begin{definition}
  Let B be a box in $G_\alpha$, define the \emph{extended neighbor} of
  B be the nine-box group centered at B.
  \begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{extB.png}
    \caption{Extended Box of B}
    \label{fig:extB}
  \end{figure}
\end{definition}
...
\begin{proof}
  ...
  \begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{packing.png}
      ...
      \note{(*)(restart) step is only
        ...
        \item There are more then two closest pairs in
          ...
          \item Disjoint case (\fref{fig:restart22}): we'll never
            ...
            \begin{figure}[h!]
              \centering
              \includegraphics[width=0.5\linewidth]{restart21.png}
              \caption{Non-disjoint case}
              \label{fig:restart21}
            \end{figure}
            \begin{figure}[h!]
              \centering
              \includegraphics[width=0.5\linewidth]{restart22.png}
